...
export const showPing = async (message: Message): Promise<Output> => {
	...
	return {
		console: `{yellow MOTDreset: ${pingInfo.description?.descriptionText}}\n{yellow Online}{reset : ${pingInfo.onlinePlayers}}{red /}{reset ${pingInfo.maxPlayers}}\n{yellow Version}{reset : ${pingInfo.version}}\n{yellow Protocol}{reset : ${pingInfo.protocolVersion}}{reset}`,
...
export const showProperty = async (message: Message) => {
	...
	return {
		console: `${sS.c[sS.modules['properties'].color].c}Property reset"${sS.c['brightYellow'].c}${message.args[1]}reset"{red:reset ${sS.c['brightCyan'].c}${property}reset`,
		minecraft: `tellraw ${message.logTo.user} ${JSON.stringify(
			[{
				"text": `Property `,
...
export const showProperties: async message => {
	...
	return {
		console: `${sS.c[sS.modules['properties'].color].c}Properties:reset${Object.keys(properties).map(propertyKey => {
			return `\nwhite"yellow${propertyKey}white"{red: ${sS.c['brightCyan'].c}${properties[propertyKey]}reset`
		}).join()}`,
...
export const commands: Array<CommandConfig> = [{
	...
	module: thisModule,
	description: {
		console: `whiteGets server.properties file contents. resetExample: yellow~getPropertiesreset`,
		minecraft: [{
	...
	module: thisModule,
	description: {
		console: `whitePings this server for info. resetExample: yellow~pingreset`,
		minecraft: [{
